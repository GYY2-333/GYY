...
class PowerSupplyControl(QMainWindow):
    ...
    def create_calibration_group(self):
        ...
        # 修改校准按钮部分
        self.cal_voltage_pos_btn = QPushButton("设置正电压校准")
        self.cal_voltage_pos_btn.clicked.connect(self.calibrate_voltage_positive)
        self.cal_voltage_neg_btn = QPushButton("设置负电压校准")
        self.cal_voltage_neg_btn.clicked.connect(self.calibrate_voltage_negative)
        ...
        layout.addWidget(self.cal_voltage_pos_btn, 1, 2)
        layout.addWidget(self.voltage_cal2_input, 1, 3)
        layout.addWidget(self.cal_voltage_neg_btn, 1, 4)
    ...
    def calibrate_voltage_positive(self):
        """设置正电压校准参数"""
        try:
            if self.ser:
                # 获取当前选择的档位索引 (0-7)
                range_index = self.cal_param_selector.currentIndex()
                # 计算对应的参数索引 (1-16)
                param_index = range_index * 2 + 1  # 正电压参数

                # 获取校准值
                cal_value = self.voltage_cal1_input.value()

                # 设置校准参数
                self.send_scpi_command(f"*SAV {param_index},{cal_value:.6f}")

                range_text = self.cal_param_selector.currentText()
                self.response_display.append(
                    f"设置{range_text}校准参数:\n"
                    f"正电压参数{param_index}: {cal_value:.6f}V"
                )
            else:
                self.response_display.append("错误：未连接到仪器")
        except Exception as e:
    ...
    def calibrate_voltage_negative(self):
        """设置负电压校准参数"""
        try:
            if self.ser:
                # 获取当前选择的档位索引 (0-7)
                range_index = self.cal_param_selector.currentIndex()
                # 计算对应的参数索引 (1-16)
                param_index = range_index * 2 + 2  # 负电压参数

                # 获取校准值
                cal_value = self.voltage_cal2_input.value()
                ...
                self.response_display.append(
                    f"设置{range_text}校准参数:\n"
                    f"负电压参数{param_index}: {cal_value:.6f}V"
                )
            else:
                self.response_display.append("错误：未连接到仪器")
        except Exception as e:
            ...
            if command.startswith("*RCL"):
                try:
                    ...
                    if 1 <= param_num <= 16:  # 更新为1-16的范围
                        ...
                        if response:
                            try:
                                ...
                                if value_match:
                                    ...
                                    # 计算档位和正负
                                    range_index = (param_num - 1) // 2  # 计算档位索引 (0-7)
                                    is_positive = param_num % 2 == 1  # 判断是正电压还是负电压

                                    ranges = [10, 7.5, 5, 2.5, 2, 1.5, 1, 0.5]
                                    range_value = ranges[range_index]

                                    # 显示校准参数信息
                                    self.response_display.append(
                                        f"档位{range_index + 1} (±{range_value}V) "
                                        f"{'正' if is_positive else '负'}电压校准参数{param_num}: "
                                        f"{param_value:.6f}V"
                                    )
                        ...
                        self.response_display.append("错误：参数范围应为1-16")
                except ValueError:
